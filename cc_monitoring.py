...
def clean_email(add_task):
    ...
    this needed when to join the file
    '''
    add_task['Email Address'] = [x.strip() for x in add_task['Email Address']]
    add_task['Email Address'] = [x.replace('@invokeisdata.com', '') for x in add_task['Email Address']]
    add_task['Email Address'] = [x.replace('hudahusna', 'huda') for x in add_task['Email Address']]
    add_task['Email Address'] = [x.replace('amishaa', 'amisha') for x in add_task['Email Address']]
    add_task['Email Address'] = [x.replace('athiyah', 'tiyah') for x in add_task['Email Address']]
    add_task['Email Address'] = [x.replace('atiqahliyana', 'atiqah') for x in add_task['Email Address']]
    return add_task 
        ...
        if cat != '':
            ...
            if add_task:
                ...
                if isinstance(a, pd.DataFrame) and isinstance(b, pd.DataFrame):
                    ...
                    if cat == 'A':
                            df['Points'] =float( (df['Calls-CR'] * 10/60) + (df['CR'] * 5) + (df['Ex_point']))
                    elif cat == 'B':
                            df['Points'] = float((df['Calls-CR'] * 10/60) + (df['CR'] * 10)+ (df['Ex_point']))
                    elif cat == 'C':
                                df['Points'] =float((df['Calls-CR'] * 10/50) + (df['CR'] * 10)+ (df['Ex_point']))
                    elif cat == 'D':
                                df['Points'] =float (df['Calls-CR'] * 0.25) + (df['CR'] * 20)+ (df['Ex_point'])
                    else:
                                df['Points'] =float((df['Calls-CR'] * 25/80) + (df['CR'] * 25)+ (df['Ex_point']))
